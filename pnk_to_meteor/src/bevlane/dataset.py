...
def _imread(path, flags=cv2.IMREAD_COLOR):
    """Read an image from a Windows path that may contain Unicode text."""
    try:
...
class BevLaneDataset(Dataset):
    def __init__(self, root, scenes, max_per_scene=None, gt_key="gt",
                 ...
                 with_command=False, boxdet_kmax=64,
                 with_depth_confidence=False):
        ...
        self.with_depth_confidence = bool(with_depth_confidence)
        if self.with_depth_confidence and not self.with_depth:
            raise ValueError("with_depth_confidence requires with_depth")
        ...
        self.boxdet_kmax = int(boxdet_kmax)
        if self.boxdet_kmax < 1:
            raise ValueError("boxdet_kmax must be positive")
        ...
        for s in scenes:
            ...
            try:                       # robust to concurrent manifest writes
                with open(mf, encoding="utf-8") as stream:
                    m = json.load(stream)
            except Exception:
                continue
    ...
    def read_image(self, scene, relative_path):
        ...
        """
        img = _imread(self.image_path(scene, relative_path))
        if img is None:
            return None
        th, tw = self.hw[scene]
    ...
    def _get_one(self, i):
        ...
        gt = _imread(os.path.join(self.root, s, f[gk]), cv2.IMREAD_GRAYSCALE)
        if gt is None:
            return None                # caller advances
        ...
        if self.use_gt_valid and f.get("gt_valid"):
            valid = _imread(os.path.join(self.root, s, f["gt_valid"]),
        ...
        if self.with_depth:
            try:
                with np.load(os.path.join(self.root, s, f["depth4"]),
                             allow_pickle=False) as z:
                    d = z["depth"].astype(np.float32)
                    dc = (z["confidence"].astype(np.uint8)
                          if "confidence" in z.files else (d > 0).astype(np.uint8))
                    if dc.shape != d.shape:
                        dc = (d > 0).astype(np.uint8)
            except Exception:
                d = np.zeros((6, 72, 128), np.float32)
                dc = np.zeros_like(d, np.uint8)
            if self.depth_hw and d.shape[1:] != tuple(self.depth_hw):
                H2, W2 = self.depth_hw
                d = np.stack([cv2.resize(c, (W2, H2), interpolation=cv2.INTER_NEAREST)
                              for c in d])
                dc = np.stack([cv2.resize(c, (W2, H2), interpolation=cv2.INTER_NEAREST)
                               for c in dc])
            if len(CAMS) == 8:
                try:
                    with np.load(os.path.join(self.root, s, f["depth4n"]),
                                 allow_pickle=False) as z:
                        dn = z["depth"].astype(np.float32)
                        dcn = (z["confidence"].astype(np.uint8)
                               if "confidence" in z.files
                               else (dn > 0).astype(np.uint8))
                        if dcn.shape != dn.shape:
                            dcn = (dn > 0).astype(np.uint8)
                    if self.depth_hw and dn.shape[1:] != tuple(self.depth_hw):
                        ...
                        dcn = np.stack([cv2.resize(c, (W2, H2),
                                        interpolation=cv2.INTER_NEAREST) for c in dcn])
                    if dn.shape[1:] != d.shape[1:]:   # stale/mismatched narrow
                        dn = np.zeros((2,) + d.shape[1:], np.float32)
                        dcn = np.zeros((2,) + d.shape[1:], np.uint8)
                except Exception:
                    dn = np.zeros((2,) + d.shape[1:], np.float32)
                    dcn = np.zeros((2,) + d.shape[1:], np.uint8)
                d = np.concatenate([d, dn], 0)
                dc = np.concatenate([dc, dcn], 0)
            ...
            if self.with_depth_confidence:
                out.append(torch.from_numpy(np.ascontiguousarray(dc[:nc])))
        if self.with_seg2d:
            try:
        ...
        if self.with_box:
            bx = _imread(os.path.join(self.root, s, f.get("bev_box", "_")),
                         cv2.IMREAD_GRAYSCALE)
            if bx is None:
