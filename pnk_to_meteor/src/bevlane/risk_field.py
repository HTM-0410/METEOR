...
def add_gauss_lobe(target, x, y, yaw, s_long, s_lat, amp, lead=0.0,
                   sigma_extent=4.0):
    """Add one lobe only inside its useful image-space bounding box.

    The previous implementation allocated and evaluated a full 400x250 field
    for every agent. At 158k PNK boxes that is billions of unnecessary exp()
    calls. Outside 4 sigma the omitted amplitude is below 3.4e-4.
    """
    cx = x + lead * np.cos(yaw)
    cy = y + lead * np.sin(yaw)
    c, s = np.cos(yaw), np.sin(yaw)
    radius_x = sigma_extent * np.sqrt((c * s_long) ** 2 + (s * s_lat) ** 2)
    radius_y = sigma_extent * np.sqrt((s * s_long) ** 2 + (c * s_lat) ** 2)
    row0 = max(0, int(np.floor((XH - (cx + radius_x)) / RES)))
    row1 = min(RH, int(np.ceil((XH - (cx - radius_x)) / RES)) + 1)
    col0 = max(0, int(np.floor((YH - (cy + radius_y)) / RES)))
    col1 = min(RW, int(np.ceil((YH - (cy - radius_y)) / RES)) + 1)
    if row0 >= row1 or col0 >= col1:
        return
    dx, dy = GX[row0:row1, col0:col1] - cx, GY[row0:row1, col0:col1] - cy
    u = c * dx + s * dy
    v = -s * dx + c * dy
    target[row0:row1, col0:col1] += amp * np.exp(
        -0.5 * ((u / s_long) ** 2 + (v / s_lat) ** 2))
...
def risk_field(boxes, count, traj, tvalid, occ_arr):
    ...
    for k in range(int(count)):
        ...
        if vru:
            sig = 1.3 + 0.8 * v
            add_gauss_lobe(inten, xe, ye, hd, sig + 0.6 * v, sig,
                           SEV_VRU, lead=0.5 * v)
        elif v < 0.35:
            add_gauss_lobe(inten, xe, ye, yaw, l * 0.55, w * 0.7, SEV_PARK)
        else:
            add_gauss_lobe(inten, xe, ye, hd, l * 0.6 + 1.2 * v, w * 0.7 + 0.3,
                           SEV_VEH, lead=0.75 * v)
